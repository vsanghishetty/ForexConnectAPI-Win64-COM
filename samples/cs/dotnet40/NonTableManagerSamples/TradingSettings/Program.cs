    ...
    class Program
    {
        ...
        private static void PrintTradingSettings(O2GSession session)
        {
            ...
            for (int i = 0; i < instrumentsReader.Count; i++)
            {
                ...
                if (instrument.Equals("Copper") || instrument.Equals("USOil") || instrument.Equals("XAU/USD"))
                {
                    double _mmr = 0;
                    double _emr = 0;
                    double _lmr = 0;

                    double dmmr = tradingSettingsProvider.getMMR(instrument, account);
                    int itype = instrumentRow.InstrumentType;
                    Console.WriteLine("Instrument = {0}, InstrumentType = {1}", instrument, itype);
